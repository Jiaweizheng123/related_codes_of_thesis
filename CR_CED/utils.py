...
def add_reverb_to_clean_audio(clean_audio, reverb_signal):
...
def get_input_features(predictorsList):
    predictors = []
    for reverb_stft_mag_features in predictorsList:
        # For CR-CED, the input feature consisted of 8 consecutive noisy
        # STFT maps: 129 × 8,
        # TODO: duration: 100ms
        inputFeatures = prepare_input_features(reverb_stft_mag_features)
...
def get_tf_feature(reverb_stft_mag_features, clean_stft_magnitude, reverb_stft_phase):
    reverb_stft_mag_features = reverb_stft_mag_features.astype(np.float32).tostring()
    clean_stft_magnitude = clean_stft_magnitude.astype(np.float32).tostring()
    reverb_stft_phase = reverb_stft_phase.astype(np.float32).tostring()

    example = tf.train.Example(features=tf.train.Features(feature={
        'reverb_stft_phase': _bytes_feature(reverb_stft_phase),
        'reverb_stft_mag_features': _bytes_feature(reverb_stft_mag_features),
